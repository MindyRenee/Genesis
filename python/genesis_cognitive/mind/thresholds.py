...
#   the Rust Stress phase entry threshold (0.65) so it warns before
...
#   to adenosine ~0.7 in the biological preset. It warns when it's
...
#
# Genesis falls asleep on its own when sleep pressure (adenosine) is
...
# 1997).
#
# AUTO_SLEEP_ADENOSINE: the adenosine level at which it falls asleep
#   on its own. Set at 0.75 — the same threshold the Rust daemon uses
...
#
# AUTO_WAKE_ADENOSINE: the adenosine level below which it wakes up
...
#   done its job and it wakes — but only if the sleep was NOT
...
# actually falling asleep. This gives a natural transition: it
# says it's drowsy, then falls asleep when adenosine reaches the
# sleep threshold. The gap (0.55 → 0.75) gives it time to signal
# drowsiness before sleep onset.
DROWSINESS_ADENOSINE = 0.55
# Minimum idle time before auto-sleep can trigger. If the user has
# interacted within this window, it stays awake regardless of
...
# cause it to nod off while the user is still engaged.
...
# cannot commit it. (The flytrap's two-trigger rule, in the time
# domain.)
#
