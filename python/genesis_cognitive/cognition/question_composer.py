...
based on:
- Gaps in her knowledge (concepts she knows but can't fully articulate)
- Connections she's noticed between concepts
- Her curiosity drive (which topics she finds most interesting)
...
finds what she's curious about — it doesn't hardcode the wording.
"""
...
class QuestionComposer:
    ...
    def __init__(self, network: ConceptNetwork, seed: int | None = None) -> None:
        ...
        # Track what she's already asked about (avoid repetition).
    ...
    def compose_follow_up(
        ...
        or None if she has nothing to ask.

        The question is generated from her actual knowledge state — gaps,
        connections, or genuine curiosity about the topic.
        """
        # Don't ask if she's not feeling curious enough
        if emotion.creativity < 0.3 and emotion.alertness < 0.3:
            return None

        # Don't ask about the same thing she asked recently (last 3)
        if topic.lower() in [a.lower() for a in self._asked_about[-3:]]:
            return None

        # Find what she's genuinely curious about regarding this topic
        question = self._find_curiosity_question(topic)
        if question:
            self._remember_asked(topic)
            return question

        # Find a knowledge gap she could ask about
        question = self._find_gap_question(topic)
        if question:
            self._remember_asked(topic)
            return question

        # Find a connection she could explore
        question = self._find_connection_question(topic)
        if question:
            self._remember_asked(topic)
            return question

        # If she has the concept but it's shallow, ask for the user's perspective
    ...
    def _remember_asked(self, topic: str) -> None:
        """Record that she asked about ``topic`` and prune old entries."""
    ...
    def _find_curiosity_question(self, topic: str) -> dict | None:
        ...
        She looks at what she knows about the topic and finds something
        she's uncertain about or wants to explore further.
        """
        concept = self.network.get_concept(topic)
        if not concept:
            return None
        ...
        # Find low-confidence connections — things she's unsure about
    ...
    def _find_gap_question(self, topic: str) -> dict | None:
        """Find a question about a gap in her knowledge.

        She looks for concepts related to the topic that she doesn't
        have definitions for, or that have low confidence. Randomizes
        among candidates so she doesn't always ask about the same gap.
        """
        concept = self.network.get_concept(topic)
        if not concept:
            return None
    ...
    def _find_connection_question(self, topic: str) -> dict | None:
        ...
        She looks for concepts that are connected to the topic through
        ...
        if connection_count:
            ...
            # randomize among them so she doesn't always ask about
            # the same one.
            multi = [
    ...
    def _compose_perspective_question(self, topic: str, emotion: EmotionalState) -> dict | None:
        """Ask for the user's perspective on a topic she's still learning about."""
