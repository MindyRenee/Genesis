...
class InternalState:
    ...
    Attributes:
        ...
        pulse_hz: Local-timer interrupt rate summed across cores
            (from daemon). The machine's actual beat — generated by
            hardware and scheduler, paused on tickless-idle cores.
        pulse: ``pulse_hz`` normalized by cores × tick rate [0,1].
        throttle_state: Passive thermal-throttle clamp applied by the
            cooling devices [0,1] — involuntary, silicon-enforced.
        top_freq_share: Fraction of the read window spent at maximum
            P-state [0,1] — sustained exertion vs. momentary arousal.
        psi_cpu / psi_io / psi_mem: Kernel pressure-stall "some"
            averages [0,1] — the body reporting resource shortfall.
        battery_cycles: Battery cycle count — lifetime wear marker.
        entropy_level: CRNG entropy pool fill [0,1].
        clocksource: Which oscillator paces the kernel clock
            (0=unknown, 1=tsc, 2=hpet, 3=acpi_pm, 4=other).
        suspend_caps: Suspend capability bits — bit0: mem suspend
            offered; bit1: RTC wakealarm present.
    """
    ...
    # Timing layer — the machine's actual beat
    pulse_hz: float = 0.0
    pulse: float = 0.0
    top_freq_share: float = 0.0
    # Involuntary layer — reflexes the body applies to the mind
    throttle_state: float = 0.0
    psi_cpu: float = 0.0
    psi_io: float = 0.0
    psi_mem: float = 0.0
    # Senescence and capability
