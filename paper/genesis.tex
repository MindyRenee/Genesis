...
thermal state; CPU frequency to arousal/pro\-cessing rate; own-process
...
Three further signal classes extend the same principle. First, timing:
local-timer interrupt counts, summed over cores from
\texttt{/proc/inter\-rupts}, map to a pulse --- the rate at which the
kernel's scheduler is actually re-entered. The counter is generated by
hardware and scheduler, so the system cannot raise it by measurement
alone, and on tickless cores it freezes when no work is dispatched;
the beat therefore reports real activity rather than elapsed wall
time. It is read machine-wide by design: the pulse is the body's
scheduling rhythm, not the process's effort. The oscillator currently
pacing the kernel is sensed as a clocksource identity (tsc, hpet,
acpi\_pm), and the share of recent time spent at the maximum P-state
reports sustained exertion, distinct from instantaneous frequency.
Second, involuntary state: the passive cooling-device clamp and the
pressure-stall averages in \texttt{/proc/pres\-sure} are computed by the
silicon and the kernel on the system's behalf but outside its control
--- reflexes the body applies to the mind, not measurements the mind
produces. Third, senescence and capability: the battery cycle count is
a literal wear odometer; the kernel entropy-pool level is sensed; and
platform suspend and wake-alarm support is probed so the system knows
which discontinuities its body permits. All channels are optional and
detected at startup; absent sensors read as zero. The sensory floor is
whatever the machine offers, not what the design assumes.
...
deprioritize, matching central autonomic network integration. Control
also extends past continuous operation: the RTC wake alarm can be
armed before suspension, so a gap in execution ends at a time the
system itself chose. Suspension remains a separate, deliberate act ---
the alarm schedules the return, it does not decide to leave.
...
\cite{dehaene2011,cowan2001}; a three-tier self model (proto-self, core self,
autobiographical self); drift-diffusion decision
